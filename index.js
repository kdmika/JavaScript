...
function deletar() {
  let nomeDoAbrigoParaDeletar = prompt(
    "Digite o nome do abrigo que deseja deletar"
  );
  for (let i = 0; i < dados.length; i++) {
...
function editar() {
  ...
  for (let i = 0; i < dados.length; i++) {
    if (nomeDoAbrigoParaEditar === dados[i].nomeAbrigo) {
      let nomeAbrigo = prompt("digite o nome do abrigo");
      let endereco = prompt("Digite o endereço do abrigo");
      let telefone = Number("Digite o telefone do abrigo");
      let capacidade = Number(prompt("Digite a capacidade do abrigo"));
      let cidade = prompt("Digite a cidade do abrigo");
...
function sair() {
  continuar = false;
  console.log("isso é um até logo!! Para reiniciar, atualize a aba do navegador.");
}
